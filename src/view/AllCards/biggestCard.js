 ...
 */
export default class BiggestCard extends Component {
  constructor(props) {
    super(props);
...
class MainContent extends Component{
  ...
  render(){
    ...
    <div >
        <div onClick={()=>{
          let list = componentList.getList("recipe")
          const randomIndex = Math.floor(Math.random() * list.length);
          this.setState({currentRecipe:list[randomIndex]})
        }}>Randomize!</div>
        <div>{this.state.currentRecipe?.getJson().name}</div>
    </div>
