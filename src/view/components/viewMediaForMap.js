  ...
  render() {
    ...
    return (
      <div style={{...styles.smallestCard, display:"flex", justifyContent:"center", alignItems:"center", flexDirection:"column", paddingTop:"20px", position:'relative'}}>
        ...
        {/* create popup that brings up card viewer of social recipe
        create button on each social recipe that copies the card to your recipe list
        add a new recipe type to your list by changing the type, owner, and _id = undefined*/}
                  <div onClick={()=>{state.opps.cleanJsonPrepareRun({addrecipe:{...this.props.obj.getJson(), type:"recipe", owner:state.user.getJson()._id, _id:undefined}})}}>Copy Recipe</div>

        <div onClick={()=>{app.setPopup({operation: "cleanPrepare", operate: "update", object: this.props.obj, currentCard:this.props.obj}, "viewSocialRecipe")}}  style={{textDecoration:"none"}}>
